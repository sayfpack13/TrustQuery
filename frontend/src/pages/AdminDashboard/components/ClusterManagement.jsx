...
export default function ClusterManagement({
  ...
  useEffect(() => {
    if (
      selectedCluster !== 'all' &&
      (!clustersList.some(c => c.name === selectedCluster) || clustersList.length === 0)
    ) {
  ...
  const handleDeleteClusterClick = (cluster) => {
    ...
    const availableTargets = clustersList.filter(c => c.name !== cluster.name);
  ...
  return (
    <>
      ...
      <section className="mb-12 p-6 bg-neutral-800 rounded-lg shadow-xl border border-neutral-700">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-3xl font-semibold text-white flex items-center">
            <FontAwesomeIcon icon={faSitemap} className="mr-3 text-purple-500" />
            Cluster Management
          </h2>
        ...
        {clustersLoading ? (
          <div className="flex justify-center items-center py-12">
            <FontAwesomeIcon icon={faSpinner} spin className="text-3xl text-purple-500 mr-3" />
            <span className="text-lg text-neutral-300">Loading clusters...</span>
          </div>
        ) : clustersList.length === 0 ? (
          <div className="text-center py-10">
            <FontAwesomeIcon icon={faSitemap} className="text-5xl text-neutral-500 mb-4" />
            <p className="text-neutral-400 text-lg mb-4">No clusters found</p>
            <p className="text-neutral-500 mb-6">Create your first cluster to organize your nodes</p>
            <button
              onClick={() => setShowCreateClusterModal(true)}
                    ...
                    <>
                      <div className="flex justify-between items-center mb-3">
                        <h3 className="text-lg font-semibold text-white flex items-center">
                          <FontAwesomeIcon 
                            icon={cluster.name === 'trustquery-cluster' ? faSitemap : faCubes} 
                            className={`mr-2 ${
                              cluster.name === 'trustquery-cluster' ? 'text-blue-400' : 'text-purple-400'
                            }`} 
                          />
                          {cluster.name}
                          {cluster.name === 'trustquery-cluster' && (
                        ...
                        <div className="flex space-x-2">
                          <button
                            ...
                            disabled={disabled || cluster.name === 'trustquery-cluster' || clusterActionLoading.includes(cluster.name)}
                            className="text-neutral-400 hover:text-white transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                            title={cluster.name === 'trustquery-cluster' ? "Cannot edit default cluster" : "Edit cluster"}
                          >
                          ...
                          <button
                            ...
                            title={cluster.name === 'trustquery-cluster' ? "Cannot delete default cluster" : "Delete cluster"}
                          >
                        ...
                        <div className="flex justify-between items-center">
                          ...
                          <div className="flex items-center">
                            <div className={`w-2 h-2 rounded-full mr-2 ${
                              cluster.nodeCount > 0 ? 'bg-green-500' : 'bg-neutral-500'
                            }`}></div>
                            <span className="text-white">{cluster.nodeCount > 0 ? 'Active' : 'Empty'}</span>
                          </div>
                        </div>
                      </div>
                      <div className="mt-4">
      ...
      <section id="nodes-section" className="mb-12 p-6 bg-neutral-800 rounded-lg shadow-xl border border-neutral-700">
        <div className="flex justify-between items-center mb-6">
          ...
          <div className="fixed inset-0 bg-black bg-opacity-75 flex justify-center items-center z-[60]">
            <div className="bg-neutral-800 p-8 rounded-lg shadow-2xl border border-neutral-600 max-w-lg w-full">
              ...
              <p className="text-neutral-300 mb-4">
                Are you sure you want to delete the cluster <span className="font-bold text-white">{clusterToDelete.name}</span>?
              </p>
              
              {clusterToDelete.nodeCount > 0 && (
                <div className="mb-6 p-4 bg-amber-900 bg-opacity-50 rounded-lg border border-amber-700">
                  <div className="flex items-start">
                    <FontAwesomeIcon icon={faExclamationCircle} className="text-amber-400 mt-1 mr-3" />
                    <div>
                      <h4 className="text-amber-200 font-medium mb-2">Cluster contains {clusterToDelete.nodeCount} nodes</h4>
                      <p className="text-amber-300 text-sm mb-3">
                        These nodes must be moved to another cluster. Please select a target cluster:
                      </p>
                      <select
                        value={targetClusterForMove}
                        onChange={(e) => setTargetClusterForMove(e.target.value)}
            ...
            {clusterLoading && (!localNodes || localNodes.length === 0) ? (
              ...
              <div className="space-y-8">
                ...
                <div className="flex justify-between items-center mb-4">
                  <h3 className="text-lg font-semibold text-white">Local Node Management</h3>
                  {localNodes && localNodes.length > 0 && (
                    <div className="flex items-center space-x-2">
                      <label className="text-sm text-neutral-300">Filter by cluster:</label>
                      <select
                                ...
                                <div>
                                  <div className="flex items-center justify-between mb-2">
                                    <div className="flex items-center">
                                      <FontAwesomeIcon 
                                        icon={faServer} 
                                        className={`mr-2 ${node.isRunning ? 'text-green-400' : 'text-neutral-400'}`} 
                                      />
